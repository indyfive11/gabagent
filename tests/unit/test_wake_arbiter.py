...
def test_two_rooms_earliest_receipt_wins(store):
    ...
    b = store.claim("raspi", now=100.05, window_secs=0.25)
    ...
    assert store.resolve(b["window_id"], "raspi", now=100.30)["verdict"] == "stand_down"


def test_detector_latency_normalization_reorders(store):
    # raspi's wake fires LATER in wall time but its detector is much slower → normalized EARLIER → it wins.
    a = store.claim("host", detector_latency_ms=0.0, now=100.00, window_secs=0.25)
    b = store.claim("raspi", detector_latency_ms=500.0, now=100.05, window_secs=0.25)
    assert store.resolve(a["window_id"], "raspi", now=100.30)["winner_room"] == "raspi"
    assert store.resolve(a["window_id"], "host", now=100.30)["verdict"] == "stand_down"


def test_media_playing_room_wins_close_call(store):
    # host is normalized-earliest, but raspi has local media playing → it must win so the turn can't be
    # grabbed out from under its playback. Still a single winner (no double-answer).
    store.claim("host", now=100.00, window_secs=0.25)
    b = store.claim("raspi", media_playing=True, now=100.10, window_secs=0.25)
    assert store.resolve(b["window_id"], "raspi", now=100.30)["winner_room"] == "raspi"
...
def test_fallback_proceeds_when_winner_never_starts(store):
    a = store.claim("host", now=100.00, window_secs=0.25)
    store.claim("raspi", now=100.05, window_secs=0.25)
    store.resolve(a["window_id"], "raspi", now=100.30)      # decide: host wins, raspi stands down
    # host never committed AND never answered → it never started → raspi un-stands-down (never-zero)
    assert store.check_fallback(a["window_id"], "raspi", now=101.30)["verdict"] == "proceed"
...
def test_fallback_stays_down_when_winner_committed(store):
    ...
    # raspi probes at +1s, host's /respond is still 6-26s away → committed keeps raspi down (presence-based)
    assert store.check_fallback(a["window_id"], "raspi", now=101.30)["verdict"] == "answered"
...
def test_mark_committed_only_the_winner(store):
    ...
    store.claim("raspi", now=100.05, window_secs=0.25)
    store.resolve(a["window_id"], "host", now=100.30)       # host wins
    assert store.mark_committed(a["window_id"], "raspi", now=100.35) is False  # loser can't commit
...
def test_heartbeat_liveness_releases_peer_when_commit_goes_stale(store):
    ...
    assert store.check_fallback(a["window_id"], "raspi", now=100.80, liveness_secs=1.0)["verdict"] == "answered"
    assert store.check_fallback(a["window_id"], "raspi", now=102.00, liveness_secs=1.0)["verdict"] == "proceed"


def test_fallback_stays_down_when_winner_answered(store):
    a = store.claim("host", now=100.00, window_secs=0.25)
    store.claim("raspi", now=100.05, window_secs=0.25)
    store.resolve(a["window_id"], "raspi", now=100.30)      # host wins
    assert store.mark_answered("host", now=100.60) is True  # host's /respond landed (terminal mark)
    assert store.check_fallback(a["window_id"], "raspi", now=102.80)["verdict"] == "answered"
...
def test_mark_answered_only_marks_a_window_this_room_won(store):
    ...
    store.claim("raspi", now=100.05, window_secs=0.25)
    store.resolve(a["window_id"], "host", now=100.30)       # host wins
    assert store.mark_answered("raspi", now=100.60) is False  # raspi didn't win any window
