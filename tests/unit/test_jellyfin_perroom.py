...
override, and play()/control() cast routing. Deployment = ONE shared host server; rooms differ only by
...
def _force_kde(monkeypatch):
    # The browser-fallback test here runs the KDE/host owned-browser path; force KDE so the new non-KDE guard
...
def test_resolve_client_target_only_falls_through_to_shared_server():
    # The real deployment: satellite sets ONLY client_target → server stays the shared host one.
    rm = {"satellite": RoomMediaProfile(jellyfin_client_target="satellite-jellyfin", duck_local=True)}
    jc = J.resolve_jellyfin(_ctx(_cfg(rm), room_id="satellite"))
...
def test_match_target_session_devicename_first_then_client():
    sessions = [
        ...
        {"Id": "B", "DeviceName": "satellite-jellyfin", "Client": "mpv-shim"},
    ]
    assert J._match_target_session(sessions, "satellite-jellyfin")["Id"] == "B"
    assert J._match_target_session(sessions, "SATELLITE-JELLYFIN")["Id"] == "B"   # case-insensitive
...
def test_session_locality_local_for_duck_local_target():
    rm = {"satellite": RoomMediaProfile(jellyfin_client_target="satellite-jellyfin", duck_local=True)}
    ctx = _ctx(_cfg(rm), room_id="satellite")
    # The room's own mpv-shim session — would read REMOTE by endpoint, but is forced LOCAL.
    s = {"DeviceName": "satellite-jellyfin", "RemoteEndPoint": "192.0.2.108:5000", "DeviceId": "x"}
    ...
    other = {"DeviceName": "Some TV", "RemoteEndPoint": "192.0.2.50:5000", "DeviceId": "y"}
    assert J._session_locality(ctx, other) == "remote"


def test_session_locality_unaffected_without_duck_local():
    rm = {"satellite": RoomMediaProfile(jellyfin_client_target="satellite-jellyfin", duck_local=False)}
    ctx = _ctx(_cfg(rm), room_id="satellite")
    s = {"DeviceName": "satellite-jellyfin", "RemoteEndPoint": "192.0.2.108:5000", "DeviceId": "x"}
...
def test_seed_remembers_target_deviceid():
    rm = {"satellite": RoomMediaProfile(jellyfin_client_target="satellite-jellyfin", duck_local=True)}
    ctx = _ctx(_cfg(rm), room_id="satellite")
    J._seed_owned_deviceid(ctx, [{"DeviceName": "satellite-jellyfin", "DeviceId": "01ad147b"}])
    assert J._owned_deviceid_cache(ctx).get("satellite") == "01ad147b"
...
def test_session_locality_pins_by_remembered_deviceid_through_flap():
    ...
    ctx = _ctx(_cfg(rm), room_id="satellite")
    J._seed_owned_deviceid(ctx, [{"DeviceName": "satellite-jellyfin", "DeviceId": "01ad147b"}])
    flapped = {"DeviceName": "", "DeviceId": "01ad147b", "RemoteEndPoint": "192.0.2.108:5000"}
...
def test_remembered_deviceid_does_not_leak_to_other_sessions():
    ...
    ctx = _ctx(_cfg(rm), room_id="satellite")
    J._seed_owned_deviceid(ctx, [{"DeviceName": "satellite-jellyfin", "DeviceId": "01ad147b"}])
    other = {"DeviceName": "Living Room TV", "DeviceId": "ffff", "RemoteEndPoint": "192.0.2.50:5000"}
    assert J._session_locality(ctx, other) == "remote"
...
def test_seed_noop_without_duck_local():
    ...
    ctx = _ctx(_cfg(rm), room_id="satellite")
    J._seed_owned_deviceid(ctx, [{"DeviceName": "satellite-jellyfin", "DeviceId": "01ad147b"}])
    assert J._owned_deviceid_cache(ctx).get("satellite") is None
...
def test_seed_refreshes_on_shim_restart():
    ...
    rm = {"satellite": RoomMediaProfile(jellyfin_client_target="satellite-jellyfin", duck_local=True)}
    ctx = _ctx(_cfg(rm), room_id="satellite")
    J._seed_owned_deviceid(ctx, [{"DeviceName": "satellite-jellyfin", "DeviceId": "old"}])
    J._seed_owned_deviceid(ctx, [{"DeviceName": "satellite-jellyfin", "DeviceId": "new"}])
    assert J._owned_deviceid_cache(ctx).get("satellite") == "new"


def test_seed_noop_for_default_room_no_target():
    """host/default (no room_id, no target) — seed is a pure no-op, cache stays empty (byte-identical)."""
...
async def test_play_casts_to_room_session(monkeypatch):
    ...
    async def fake_sessions(jc):
        return [{"Id": "S1", "DeviceName": "satellite-jellyfin"}]
    ...
    assert res.success and called == {"session_id": "S1", "item_id": "movie42", "label": "satellite-jellyfin"}
...
async def test_play_no_target_uses_browser(monkeypatch):
    ctx = _ctx(_cfg(), room_id=None)   # host / default: no per-room target
    used = {}
