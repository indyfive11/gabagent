...
# packaging model (INSTALL_PLAN §4/§9). The install path imports `installkit` from the vendored copy at the
# repo root (installkit/); nothing depends on the sibling checkout at runtime, and the copy is git-tracked so
...
# tree against installkit@INSTALLKIT_PIN in INSTALLKIT_SRC — a local checkout OR a fresh CI clone that
# contains the pinned commit. Wired as a blocking CI gate in .github/workflows/vendor-check.yml.
#
# Format mirrors voice-agent's Makefile verbatim (GA↔VAC alignment 2026-07-27, decision (b)) so both
# consumers read the same pin the same way — one mechanism, no drift between repos.
